...
const EventDetailsPage: React.FC = () => {
  ...
  const handleRegister = async () => {
    try {
      await dispatch(
        updateEventRegistration({ eventId: event.id!, isRegistered: true })
      );
  ...
  return (
    <div className="container mx-auto p-4">
            ...
            {isWhenWhereExpanded && (
              <div className="mt-2">
                <p className="text-gray-700">
                  {event.venue.address1}, {event.venue.city},{" "}
      ...
      {/* What's About Event */}
      <div className="mb-4">
        <div className="w-full text-left text-lg font-semibold text-blue-800">
          WHAT'S ABOUT EVENT
        </div>
        {isAboutExpanded && (
        ...
        <button
          onClick={() => setIsAboutExpanded((prevState) => !prevState)}
      ...
      <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-5 mb-4">
        <button
          ...
          className={`p-2 ${
            isRegistered ? "bg-gray-400" : "bg-blue-500"
          } text-white rounded`}
        >
